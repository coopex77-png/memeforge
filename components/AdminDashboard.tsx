...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const [newDiscountPercentage, setNewDiscountPercentage] =
    useState<number>(10);
  const [newDiscountDurationValue, setNewDiscountDurationValue] = useState<
    number | ""
  >("");
  const [newDiscountDurationUnit, setNewDiscountDurationUnit] = useState<
    "m" | "h" | "d" | "w"
  >("d");
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            setUsers((prev) =>
              prev.map((u) =>
                u.access_code === payload.new.access_code
                  ? (payload.new as DatabaseUser)
                  : u,
              ),
            );
          } else if (payload.eventType === "DELETE") {
            setUsers((prev) =>
              prev.filter((u) => u.access_code !== payload.old.access_code),
            );
          }
        },
      )
  ...
  const handleCreateDiscountCode = async (e: React.FormEvent) => {
    ...
    let expiresAt = null;
    if (
      typeof newDiscountDurationValue === "number" &&
      newDiscountDurationValue > 0
    ) {
      const date = new Date();
      if (newDiscountDurationUnit === "m")
        date.setMinutes(date.getMinutes() + newDiscountDurationValue);
      else if (newDiscountDurationUnit === "h")
        date.setHours(date.getHours() + newDiscountDurationValue);
      else if (newDiscountDurationUnit === "d")
        date.setDate(date.getDate() + newDiscountDurationValue);
      else if (newDiscountDurationUnit === "w")
        date.setDate(date.getDate() + newDiscountDurationValue * 7);
    ...
    try {
      ...
      if (error) {
        alert(
          "Error creating discount code. Make sure it is unique. " +
            error.message,
        );
      } else {
        setNewDiscountCode("");
    ...
    try {
      ...
      if (error) {
        ...
        setDiscountCodes((prev) =>
          prev.filter((dc) => dc.id !== discountToDelete),
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleToggleDiscount = async (id: string, currentValue: boolean) => {
    ...
    if (!error) {
      setDiscountCodes((prev) =>
        prev.map((dc) =>
          dc.id === id ? { ...dc, is_active: !currentValue } : dc,
        ),
      );
    }
  };
  ...
  const handleUpdateCredits = async (
    accessCode: string,
    field: "art_credits" | "lore_credits",
    action: "add" | "sub",
  ) => {
  ...
  const handleSaveAccessCode = async (oldCode: string) => {
    if (
      !editingCode ||
      !editingCode.new.trim() ||
      editingCode.new === oldCode
    ) {
      setEditingCode(null);
      return;
    }
    ...
    try {
      const { error } = await supabase
        .from("users")
        .update({ access_code: editingCode.new })
        .eq("access_code", oldCode);
      if (error) {
  ...
  const isSubscriptionExpired = (user: DatabaseUser) => {
    if (user.is_admin || !user.subscription_days || !user.subscription_start)
      return false;
    const start = new Date(user.subscription_start).getTime();
  ...
  const filteredUsers = users.filter((u) => {
    const matchesSearch = u.access_code
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    const matchesPackage =
      packageFilter === "All" ||
      (u.package_name || "Starter") === packageFilter;
  ...
  return (
    <div className="admin-layout">
      ...
      <aside className="admin-sidebar">
        <div className="admin-sidebar-header flex-col items-start gap-1 mb-8 mt-2">
          <img
            src="/pepe_forge.png"
            alt="Forge Logo"
            className="h-[45px] w-auto object-contain drop-shadow-xl"
            onError={(e) => {
              e.currentTarget.onerror = null;
              e.currentTarget.src =
                "https://placehold.co/200x65/transparent/DEFD41?text=FORGE";
            }}
          />
          <span className="admin-badge ml-1 mt-1">Admin Panel</span>
        </div>
        <nav className="admin-nav">
          ...
          >
            <div
              className={`w-1 pl-0 h-8 absolute left-0 rounded-r-md transition-all ${activeTab === "overview" ? "bg-accent shadow-[0_0_10px_#DEFD41]" : "bg-transparent"}`}
            ></div>
            <svg
          ...
          >
            <div
              className={`w-1 pl-0 h-8 absolute left-0 rounded-r-md transition-all ${activeTab === "users" ? "bg-accent shadow-[0_0_10px_#DEFD41]" : "bg-transparent"}`}
            ></div>
            <svg
              className="w-5 h-5 opacity-80"
              fill="none"
            ...
            {users.length > 0 && (
              <span className="px-2 py-0.5 rounded-md bg-white/10 text-xs font-bold font-mono">
                {users.length}
              </span>
            )}
          </button>
          ...
          >
            <div
              className={`w-1 pl-0 h-8 absolute left-0 rounded-r-md transition-all ${activeTab === "discounts" ? "bg-accent shadow-[0_0_10px_#DEFD41]" : "bg-transparent"}`}
            ></div>
            <svg
              className="w-5 h-5 opacity-80"
              fill="none"
      ...
      <main className="admin-main">
        {activeTab === "overview" && (
          <AdminOverview
            users={users}
            getBadgeForPackage={getBadgeForPackage}
                ...
                >
                  <div className="flex-1 min-w-[200px]">
                    ...
                    <div className="flex bg-[rgba(0,0,0,0.3)] border border-white/10 rounded-lg overflow-hidden focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/10 transition-all">
                      <input
                        ...
                        className="w-full bg-transparent text-white px-4 py-2.5 outline-none uppercase tracking-widest font-mono text-sm"
                      ...
                      <button
                        ...
                        className="px-4 bg-white/5 hover:bg-white/10 text-accent font-bold text-lg border-l border-white/10 transition-colors"
                  ...
                  <div className="w-28">
                    ...
                    <input
                      type="number"
                      value={newArtCredits}
                      onChange={(e) => setNewArtCredits(Number(e.target.value))}
                      className="admin-input w-full py-2.5 font-mono"
                      required
                    />
                  </div>
                  <div className="w-28">
                    ...
                    <input
                      type="number"
                      value={newLoreCredits}
                      onChange={(e) =>
                        setNewLoreCredits(Number(e.target.value))
                      }
                      className="admin-input w-full py-2.5 font-mono"
                      required
                    />
                  ...
                  <div className="w-40 shrink-0">
                    ...
                    <select
                      ...
                      className="admin-select w-full py-2.5"
                    >
                  ...
                  <button
                    type="submit"
                    className="admin-btn admin-btn-accent py-2.5 px-8 text-black"
                  >
                    Create User
                  </button>
                </form>
                      ...
                      <tr key={user.access_code} className="group">
                        <td className="pl-6 font-mono text-white font-medium">
                          {editingCode?.old === user.access_code ? (
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                onClick={() =>
                                  handleSaveAccessCode(user.access_code)
                                }
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() =>
                                  setEditingCode({
                                    old: user.access_code,
                                    new: user.access_code,
                                  })
                                }
                        ...
                        <td>
                          {user.is_admin ? (
                            ...
                            <span className="badge badge-starter">USER</span>
                          )}
                        </td>
                        <td>
                          <select
                        ...
                        <td className="text-center">
                          <label className="admin-toggle mx-auto">
                            <input
                              type="checkbox"
                              checked={user.can_use_news_scrape || false}
                              onChange={() =>
                                handleToggle(
                                  user.access_code,
                                  "can_use_news_scrape",
                                  user.can_use_news_scrape || false,
                                )
                              }
                              disabled={user.is_admin}
                            />
                        ...
                        <td className="text-center">
                          {user.is_admin ? (
                          ...
                          ) : editingCredits?.access_code ===
                              user.access_code &&
                            editingCredits.type === "art" ? (
                            ...
                            <div className="inline-flex items-center bg-black/40 border border-white/10 rounded overflow-hidden">
                              <button
                                ...
                                className="px-2 py-1 text-slate-400 hover:text-white hover:bg-white/10 font-bold transition-colors"
                              >
                                -
                              </button>
                              <span
                                onClick={() =>
                                  setEditingCredits({
                                    ...
                                    type: "art",
                                    value: user.art_credits.toString(),
                                  })
                                }
                              ...
                              <span
                                onClick={() =>
                                  setEditingCredits({
                                    access_code: user.access_code,
                                    type: "lore",
                                    value: user.lore_credits.toString(),
                                  })
                                }
                                className="w-10 text-center font-mono text-sm cursor-pointer hover:text-accent font-bold"
                                title="Click to edit"
                          ...
                          {user.is_admin ? (
                            ...
                            <div className="flex flex-col items-center gap-1">
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    handleSaveSub(user.access_code)
                                  }
                                  className="text-green-400 hover:text-green-300 text-[10px] font-bold tracking-widest uppercase"
                                >
                                  Save
                                </button>
                                <button
                                  onClick={() => setEditingSub(null)}
                                  className="text-slate-400 hover:text-white text-[10px] font-bold tracking-widest uppercase"
                                >
                                  Cancel
                                </button>
                              </div>
                            ...
                            <div
                              className="flex flex-col items-center group/sub relative cursor-pointer"
                              onClick={() => {
                ...
                >
                  <div className="flex-1 min-w-[200px]">
                    ...
                    <input
                      ...
                      className="admin-input w-full py-2.5 font-mono uppercase tracking-widest text-sm"
                  ...
                  <div className="w-32">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="admin-input w-full py-2.5 font-mono pr-8"
                        required
                      />
                  ...
                  <div className="w-[260px]">
                    ...
                    <div className="flex bg-[rgba(0,0,0,0.3)] border border-white/10 rounded-lg overflow-hidden focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/10 transition-all">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setNewDiscountDurationUnit(e.target.value as any)
                        }
                      ...
                      return (
                        ...
                        >
                          <td className="pl-6 font-mono text-accent font-bold tracking-widest">
                            {dc.code}
                            {isExpired && (
                              <span className="ml-3 badge badge-expired">
                                Expired
                              </span>
                            )}
                          </td>
                          ...
                          <td className="text-center">
                            <label className="admin-toggle mx-auto">
                              <input
                                ...
                                onChange={() =>
                                  handleToggleDiscount(dc.id, dc.is_active)
                                }
                              />
                              <span className="admin-toggle-slider"></span>
                            </label>
                          ...
                          <td className="text-slate-400 font-mono text-xs">
                            {dc.expires_at ? (
                              new Date(dc.expires_at).toLocaleString()
                            ) : (
        ...
        <div className="admin-modal-backdrop">
          <div className="admin-modal-content">
            ...
            <p className="text-slate-400 mb-8 text-center text-sm leading-relaxed">
              ...
              <strong className="text-accent bg-accent/10 px-1 py-0.5 rounded font-mono">
                {userToDelete}
              </strong>
              ?<br />
