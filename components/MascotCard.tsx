...
import { AVAILABLE_ART_STYLES, modifyMascotImage } from '../services/geminiService';
...
export const MascotCard: React.FC<MascotCardProps> = ({ mascot, onSelect, onImageClick, onGenerateScenes, onEdit, onUpdateMascot, isGenerating = false }) => {
    ...
    const calculatedCredits = React.useMemo(() => {
        ...
        for (const model of selectedModels) {
            req += count * (model === 'pro' ? 4 : 2);
        }
        if (includeDex) req += 6;
        if (includeXComm) req += 6;
        return req;
    ...
    const handleSaveEdit = async (e: React.MouseEvent) => {
        e.stopPropagation();
        if (!onUpdateMascot) return;

        let finalImageUrl = mascot.imageUrl;

        if (editPrompt.trim()) {
            setIsRegenerating(true);
            try {
                finalImageUrl = await modifyMascotImage(mascot.imageUrl, editPrompt);
            } catch (err) {
                console.error("Image Edit Failed", err);
                alert("Failed to regenerate image. Lore will still be updated.");
            } finally {
    ...
    return (
        <div className="group relative flex flex-col h-full bg-navy-900 border border-white/5 rounded-xl overflow-hidden transition-all duration-300 hover:border-accent/40 hover:shadow-[0_0_20px_rgba(222,253,65,0.05)]">
            ...
            >
                {isLoading ? (
                    ...
                    <>
                        ...
                        {/* Edit Overlay (Image Part) */}
                        {isEditing && (
                            <div className="absolute inset-0 z-30 bg-black/80 backdrop-blur-md p-4 flex flex-col pt-12" onClick={e => e.stopPropagation()}>
                                <label className="text-[10px] font-black text-accent uppercase tracking-widest mb-2">Image Modification</label>
                                <textarea
                                    value={editPrompt}
                                    onChange={e => setEditPrompt(e.target.value)}
                                    placeholder="e.g. 'Add a crown', 'Change color to blue'..."
                                    className="flex-1 bg-navy-950 border border-white/10 rounded-lg p-3 text-xs text-white placeholder:text-slate-600 outline-none focus:border-accent resize-none font-mono"
                                />
                                <div className="mt-3 flex gap-2">
                                    <button onClick={handleSaveEdit} disabled={isRegenerating} className="flex-1 bg-accent text-black font-black py-2 rounded text-[10px] tracking-widest uppercase hover:bg-white transition-colors">
                                        {isRegenerating ? 'PROCESSING...' : 'APPLY EDITS'}
                                    </button>
                                    <button onClick={handleCancelEdit} disabled={isRegenerating} className="px-3 bg-white/5 text-white border border-white/10 font-bold py-2 rounded text-[10px] uppercase hover:bg-white/10">ESC</button>
                                </div>
                            </div>
                        )}
                                ...
                                <div className="space-y-3" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    {calculatedCredits > 0 && (
                                        <div className="flex justify-between items-center text-[10px] font-bold mt-1 px-1">
                                            <span className="text-slate-400 uppercase tracking-widest">Total Cost:</span>
                                            <span className="text-accent">{calculatedCredits} Credits</span>
                                        </div>
                                    )}
                        ...
                        {isRegenerating && (
                            <div className="absolute inset-0 z-[40] bg-black/60 backdrop-blur-xl flex flex-col items-center justify-center p-6 text-center">
                                <div className="w-12 h-12 border-4 border-accent/20 border-t-accent rounded-full animate-spin mb-4"></div>
                                <h4 className="text-white font-black text-sm tracking-tighter uppercase">Regenerating Image</h4>
                                <p className="text-accent text-[9px] font-mono mt-1 animate-pulse uppercase">AI Workspace Active</p>
                            </div>
                        )}
            ...
            <div className="flex flex-col flex-1 p-4 bg-navy-900 z-20 relative">
                <div className="mb-2">
                    {isEditing ? (
                        <div className="flex flex-col gap-2 mb-2" onClick={e => e.stopPropagation()}>
                            {/* Inputs removed: Name/Ticker are inferred from Lore or empty */}
                        </div>
                    ) : (
                        <>
                            <div className="flex items-center gap-2 mb-1.5 h-4"> {/* Fixed height to maintain layout if empty */}
                                {mascot.ticker && mascot.ticker !== "$UNKNOWN" && !isLoading && (
                            ...
                            <h3 className="text-base font-black text-white leading-tight group-hover:text-accent transition-colors tracking-tight truncate min-h-[20px]">
                ...
                <div className="text-[11px] text-slate-400 font-medium leading-relaxed mb-3">
                    {isEditing ? (
                        <textarea
                            value={editNarrative}
                            onChange={e => setEditNarrative(e.target.value)}
                            placeholder="Describe the lore..."
                            className="w-full bg-navy-950 border border-white/10 rounded-lg p-3 text-[10px] text-slate-300 outline-none focus:border-accent resize-none font-medium h-20"
